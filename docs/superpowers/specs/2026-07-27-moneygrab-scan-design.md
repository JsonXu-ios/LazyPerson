...
## 筛选逻辑（v2 分档规则，与 K 线图粗线口径一致）

> v1 的"20%区间上半段"规则已废弃（覆盖面过宽且会放进未站上20%的股票）。

90 日窗口视为一个波段（低点→高点，时间顺序）。对每只股票：

1. 取近 **90 自然日** 的日线（与前端 `sliceDailyPayloadByCalendarDays` + `computeAutoDrawing` 口径一致，qfq 复权）。
2. `low90 = 窗口内最低 low`，其所在日为波段低点日；**低点若是最后一天则不入选**（没有低点→高点的波段）。
3. `pct = (最新价 / low90 − 1) × 100`，最新价取盘中实时快照价（收盘后即收盘价）。
4. **八档分组**，第 k 档阈值 `F = 20 + 30×(k−1)`（20/50/80/110/140/170/200/230），"先过"线 `L = F − 10`（10/40/70/…/220）：
   - 命中第 k 档：`pct ≥ F`，取满足的最高档（超过 230% 归第八档）；
   - 记录波段低点日与低点之后首次收盘站上 L 线的"过线日"（时间顺序：低点 → 过L → 最后一天≥F）。
5. 输出按档分组展示，组内按超出阈值幅度排序；同时展示低点日/过线日。
